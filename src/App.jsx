    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:300,padding:16}}>
      <div style={{background:t.card,borderRadius:24,padding:24,width:"100%",maxWidth:320}}>
        ...
        <div style={{display:"flex",gap:8,marginBottom:16}}>
          <div style={{flex:1}}>
            <div style={{fontSize:11,fontWeight:700,color:t.muted,marginBottom:4}}>WEIGHT (KG)</div>
            <input type="text" inputMode="decimal" value={weight} onChange={e=>setWeight(e.target.value)} placeholder="e.g. 100" style={{width:"100%",padding:"10px",borderRadius:10,border:`1px solid ${t.border}`,fontSize:15,color:t.text,background:t.bg,outline:"none",boxSizing:"border-box",textAlign:"center"}}/>
          </div>
          <div style={{flex:1}}>
            <div style={{fontSize:11,fontWeight:700,color:t.muted,marginBottom:4}}>REPS</div>
            <input type="text" inputMode="numeric" value={reps} onChange={e=>setReps(e.target.value)} placeholder="e.g. 8" style={{width:"100%",padding:"10px",borderRadius:10,border:`1px solid ${t.border}`,fontSize:15,color:t.text,background:t.bg,outline:"none",boxSizing:"border-box",textAlign:"center"}}/>
          </div>
        </div>
        {orm&&(<>
          <div style={{background:t.accent+"18",borderRadius:12,padding:"12px",textAlign:"center",marginBottom:12}}>
            <div style={{fontSize:12,fontWeight:700,color:t.accent,marginBottom:2}}>ESTIMATED 1RM</div>
            <div style={{fontSize:32,fontWeight:800,color:t.accent}}>{orm}kg</div>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
            {pcts.map(p=>(
              <div key={p} style={{background:t.bg,borderRadius:10,padding:"8px",textAlign:"center"}}>
                <div style={{fontSize:11,color:t.muted,fontWeight:600}}>{p}%</div>
                <div style={{fontSize:14,fontWeight:700,color:t.text}}>{+(orm*p/100).toFixed(1)}kg</div>
              </div>
            ))}
          </div>
        </>)}
  ...
  return(
    <div style={{padding:"0 16px 100px",maxWidth:480,margin:"0 auto"}}>
      <div style={{paddingTop:16,paddingBottom:16,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div><div style={{fontSize:13,color:t.muted,fontWeight:500}}>{greeting()}</div><div style={{fontSize:22,fontWeight:800,color:t.text}}>{userName||"Terry Day"}</div></div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
        <Card style={{marginBottom:0}}>
          ...
          <Sparkline data={bwLog.slice(-7).map(e=>e.w)} color={t.accent}/>
        </Card>
        <Card style={{marginBottom:0}}>
...
function MoreTab({t,toggleDark,log,userName,setUserName}){
  ...
  // Monthly report
  const getMonthly=()=>{
    const now=new Date();const mo={Jan:0,Feb:1,Mar:2,Apr:3,May:4,Jun:5,Jul:6,Aug:7,Sep:8,Oct:9,Nov:10,Dec:11};
    const thisMonth=log.filter(e=>{const p=e.date.split(" ");const d=new Date(+p[2],mo[p[1]],+p[0]);return d.getMonth()===now.getMonth()&&d.getFullYear()===now.getFullYear();});
    const totalSets=thisMonth.reduce((a,e)=>a+e.sets.filter(s=>!s.warmup).length,0);
    const totalVol=thisMonth.reduce((a,e)=>a+e.sets.filter(s=>!s.warmup&&s.weight&&!s.bodyweight).reduce((b,s)=>b+(+s.weight*(+s.reps||0)),0),0);
    const sessions=[...new Set(thisMonth.map(e=>e.date))].length;
    const prs=thisMonth.filter(e=>e.isPR).length;
    const totalCals=thisMonth.reduce((a,e)=>a+(e.calories||0),0);
    return{totalSets,totalVol:Math.round(totalVol),sessions,prs,totalCals:Math.round(totalCals)};
  };

  if(view==="monthly"){
    const {totalSets,totalVol,sessions,prs,totalCals}=getMonthly();
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
          {[{label:"Sessions",val:sessions,icon:"📅"},{label:"Working sets",val:totalSets,icon:"💪"},{label:"Total volume",val:`${totalVol.toLocaleString()}kg`,icon:"⬆️"},{label:"New PRs",val:prs,icon:"🏆"},{label:"Cals burned",val:`~${totalCals}`,icon:"🔥"}].map(({label,val,icon})=>(
            <Card key={label} style={{marginBottom:0,textAlign:"center"}}>
              ...
              <div style={{fontSize:11,color:t.muted,fontWeight:600,textTransform:"uppercase",letterSpacing:"0.06em"}}>{label}</div>
            </Card>
          ))}
        </div>
        <Card>
          ...
          {SPLIT.map(cat=>{
            ...
            const now=new Date();
            const catSets=log.filter(e=>{const p=e.date.split(" ");const d=new Date(+p[2],mo[p[1]],+p[0]);return d.getMonth()===now.getMonth()&&d.getFullYear()===now.getFullYear()&&(e.category===cat||(cat==="Arms & Shoulders"&&(e.category==="Arms"||e.category==="Shoulders")));}).reduce((a,e)=>a+e.sets.filter(s=>!s.warmup).length,0);
            return(
              <div key={cat} style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:`1px solid ${t.border}`}}>
                <span style={{fontSize:13,color:t.text,fontWeight:600}}>{cat}</span>
                <span style={{fontSize:13,fontWeight:700,color:CAT_COLORS[cat]}}>{catSets} sets</span>
              </div>
            );
          })}
        </Card>
          ...
          <Card>
            ...
            {measurements.slice(0,5).map((m,i)=>(
              <div key={i} style={{padding:"8px 0",borderBottom:`1px solid ${t.border}`}}>
                <div style={{fontSize:12,fontWeight:700,color:t.muted,marginBottom:4}}>{m.date}</div>
                <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                  {fields.filter(f=>m[f]).map(f=><span key={f} style={{fontSize:11,background:t.bg,color:t.text,padding:"2px 8px",borderRadius:99,fontWeight:600}}>{f}: {m[f]}cm</span>)}
                </div>
              </div>
            ))}
          </Card>
  ...
  if(view==="bodyweight")return(
    <div style={{padding:"0 16px 100px",maxWidth:480,margin:"0 auto"}}>
      ...
      <Card>
        ...
        <div style={{display:"flex",gap:8}}>
          <input value={bwInput} onChange={e=>setBwInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&logBW()} type="text" inputMode="decimal" placeholder="e.g. 85.5" style={{flex:1,padding:"10px 12px",borderRadius:12,border:`1px solid ${t.border}`,fontSize:15,fontWeight:600,color:t.text,background:t.bg,outline:"none"}}/>
      ...
      <Card>
        ...
        {bwLog.length===0?<div style={{fontSize:13,color:t.muted}}>No entries yet.</div>:[...bwLog].reverse().map((e,i)=>(
          <div key={i} style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:`1px solid ${t.border}`}}>
            <span style={{fontSize:13,color:t.muted}}>{e.d}</span><span style={{fontSize:14,fontWeight:700,color:t.text}}>{e.w}kg</span>
          </div>
        ))}
      </Card>
    </div>
  ...
  if(view==="history"){
    if(histEx){
      const hist=getExHistory(histEx);const weights=hist.filter(e=>e.best).map(e=>e.best);
      return(
        <div style={{padding:"0 16px 100px",maxWidth:480,margin:"0 auto"}}>
          ...
          {hist.map((h,i)=>(<Card key={i}><div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{fontSize:13,fontWeight:700,color:t.text}}>{h.date}</span>{h.best&&<span style={{fontSize:13,fontWeight:800,color:t.accent}}>{h.best}kg best</span>}</div><div style={{fontSize:12,color:t.muted}}>{h.sets} sets · reps: {h.reps}{h.duration?` · ${fmtTime(h.duration)}`:""}</div></Card>))}
        </div>
      );
    }
    return(
      <div style={{padding:"0 16px 100px",maxWidth:480,margin:"0 auto"}}>
        ...
        {loggedExercises.length===0?<div style={{textAlign:"center",color:t.muted,marginTop:40,fontSize:14}}>No exercises logged yet.</div>
          :loggedExercises.map(ex=>{
            ...
            const pr=sessions.reduce((best,e)=>e.sets.reduce((b,s)=>(!s.bodyweight&&!s.warmup&&+s.weight>b?+s.weight:b),best),0);
  ...
  if(view==="settings")return(
    <div style={{padding:"0 16px 100px",maxWidth:480,margin:"0 auto"}}>
      <BackBtn onClick={()=>setView("menu")} label="More"/>
      <div style={{fontSize:22,fontWeight:800,color:t.text,marginBottom:16}}>Settings</div>
      <Card>
        <div style={{fontSize:13,fontWeight:700,color:t.text,marginBottom:8}}>Your name</div>
        <div style={{display:"flex",gap:8}}>
          <input value={nameInput} onChange={e=>setNameInput(e.target.value)} placeholder="Your name" style={{flex:1,padding:"10px 12px",borderRadius:12,border:`1px solid ${t.border}`,fontSize:14,color:t.text,background:t.bg,outline:"none"}}/>
          <button onClick={()=>setUserName(nameInput)} style={{padding:"10px 16px",borderRadius:12,border:"none",background:t.accent,color:"#fff",fontWeight:700,fontSize:14,cursor:"pointer"}}>Save</button>
        </div>
      </Card>
    </div>
  );
...
function Planner({exercises,setExercises,t}){
  ...
  const dupPlan=(plan)=>{const p={...plan,id:Date.now(),name:plan.name+" (copy)",created:new Date().toLocaleDateString("en-GB",{day:"2-digit",month:"short",year:"numeric"})};setPlans(prev=>[...prev,p]);};
        ...
        return(<Card key={plan.id}>
          <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:8}}>
            ...
            <div style={{display:"flex",gap:6,flexShrink:0}}>
              ...
              <button onClick={()=>dupPlan(plan)} style={{background:t.bg,border:`1px solid ${t.border}`,borderRadius:8,padding:"5px 10px",fontSize:12,fontWeight:600,color:t.accent,cursor:"pointer"}}>Copy</button>
              <button onClick={()=>{if(window.confirm("Delete this plan?"))setPlans(p=>p.filter(x=>x.id!==plan.id));}} style={{background:"#fff0f0",border:"1px solid #fecaca",borderRadius:8,padding:"5px 10px",fontSize:12,fontWeight:600,color:"#ef4444",cursor:"pointer"}}>Del</button>
            </div>
          </div>
  ...
  return(
    <div style={{padding:"0 16px 100px",maxWidth:480,margin:"0 auto"}}>
      ...
      {selectedExs.length>1&&(
        <div style={{background:t.bg,borderRadius:12,padding:"12px 14px",marginBottom:12}}>
          <div style={{fontSize:12,fontWeight:700,color:t.muted,marginBottom:8}}>SUPERSETS (optional) — link two exercises together</div>
          {selectedExs.map((k,i)=>{
            if(i%2!==0)return null;
            const next=selectedExs[i+1];if(!next)return null;
            const paired=supersets[k]===next;
            return(<div key={k} onClick={()=>setSupersets(p=>paired?Object.fromEntries(Object.entries(p).filter(([x])=>x!==k)):{...p,[k]:next})} style={{display:"flex",alignItems:"center",gap:8,padding:"8px",borderRadius:10,cursor:"pointer",background:paired?t.accent+"18":t.card,border:`1px solid ${paired?t.accent:t.border}`,marginBottom:6}}>
              <div style={{width:16,height:16,borderRadius:"50%",border:`2px solid ${paired?t.accent:t.border}`,background:paired?t.accent:"transparent",flexShrink:0}}/>
              <span style={{fontSize:12,color:t.text}}>{k.split("||")[1]} + {next.split("||")[1]}</span>
            </div>);
          })}
        </div>
      )}
...
export default function App(){
  ...
  const [userName,setUserName]=useState(()=>load("pt_username","Terry Day"));
  ...
  useEffect(()=>{
    if(workoutStart){timerRef.current=setInterval(()=>setElapsed(Math.floor((Date.now()-workoutStart)/1000)),1000);}
    else{clearInterval(timerRef.current);setElapsed(0);}
    return()=>clearInterval(timerRef.current);
  },[workoutStart]);

  const cc=activeCat?CAT_COLORS[activeCat]:null;
  const getPR=ex=>{const e=log.filter(e=>e.exercise===ex);if(!e.length)return null;let b=0;e.forEach(e=>e.sets.forEach(s=>{if(!s.bodyweight&&!s.warmup&&+s.weight>b)b=+s.weight;}));return b>0?b:null;};
  const plans=load("pt_plans",[]);
  ...
  const openEx=(cat,ex)=>{
    ...
    setTargetReps(tgt[ex]?.reps||"");setTargetSets(tgt[ex]?.sets||"");
    ...
    if(wv==="active"&&activePlan){
      ...
      return(
        <div style={{padding:"0 16px 100px",maxWidth:480,margin:"0 auto"}}>
          ...
          {activePlan.exercises.map((key)=>{
            const [cat,ex]=key.split("||"),col=CAT_COLORS[cat],done=planDone[key],pr=getPR(ex);
            const isSuper=activePlan.supersets&&activePlan.supersets[key];
            return(
              <div key={key}>
          ...
          {allDone&&(<div style={{textAlign:"center",marginTop:16,padding:"20px 0"}}>
            <div style={{fontSize:36,marginBottom:8}}>🎉</div>
            <div style={{fontSize:18,fontWeight:800,color:t.text,marginBottom:4}}>Workout complete!</div>
            <div style={{fontSize:13,color:t.muted,marginBottom:4}}>All {total} exercises done</div>
            <div style={{fontSize:15,fontWeight:700,color:t.accent,marginBottom:16}}>{fmtTime(elapsed)}</div>
            <button onClick={()=>{setActivePlan(null);setPlanDone({});setWorkoutStart(null);setWv("list");}} style={{padding:"13px 32px",borderRadius:14,border:"none",background:"#22c55e",color:"#fff",fontWeight:800,fontSize:15,cursor:"pointer"}}>Finish ✓</button>
          </div>)}
        </div>
      );
    }
      ...
      return(
        <div style={{padding:"0 16px 100px",maxWidth:480,margin:"0 auto"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",paddingTop:16,paddingBottom:8}}>
            <button onClick={()=>setWv(activePlan?"active":"list")} style={{background:"none",border:"none",color:t.accent,fontWeight:700,fontSize:14,cursor:"pointer",display:"flex",alignItems:"center",gap:4}}>
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><line x1="19" y1="12" x2="5" y2="12"/><polyline points="12 19 5 12 12 5"/></svg>
          ...
          <div style={{background:t.dark?"#1e3a5f":"#f0f7ff",borderRadius:12,padding:"10px 14px",marginBottom:12,display:"flex",gap:10,alignItems:"center"}}>
            ...
            {targetSets&&targetReps&&<div style={{fontSize:11,fontWeight:700,color:"#22c55e",marginLeft:"auto"}}>{workingSets.length}/{targetSets} done</div>}
          </div>
  ...
  const LogContent=()=>(
    <div style={{padding:"0 16px 100px",maxWidth:480,margin:"0 auto"}}>
      ...
      {log.length===0?<div style={{textAlign:"center",color:t.muted,marginTop:60,fontSize:14}}>No workouts logged yet.</div>
        :log.map(entry=>{
          const col=CAT_COLORS[entry.category];
          const isSwipe=swipeId===entry.id;
          return(
            <div key={entry.id} style={{position:"relative",marginBottom:10,overflow:"hidden",borderRadius:t.radius}}>
              {/* Delete button behind */}
              <div style={{position:"absolute",right:0,top:0,bottom:0,width:80,background:"#ef4444",display:"flex",alignItems:"center",justifyContent:"center",borderRadius:`0 ${t.radius}px ${t.radius}px 0`,cursor:"pointer"}} onClick={()=>{setLog(p=>p.filter(e=>e.id!==entry.id));setSwipeId(null);}}>
                <span style={{color:"#fff",fontWeight:700,fontSize:13}}>Delete</span>
              </div>
              {/* Card */}
              <div style={{background:t.card,borderRadius:t.radius,padding:"16px",transform:isSwipe?"translateX(-80px)":"translateX(0)",transition:"transform 0.25s ease",boxShadow:t.dark?"none":"0 1px 4px rgba(0,0,0,0.07)",border:t.dark?`1px solid ${t.border}`:"none",position:"relative"}}
                ...
                <div style={{fontSize:10,color:t.muted,marginTop:6,textAlign:"right"}}>Swipe left to delete</div>
              </div>
            </div>
          );
        })}
